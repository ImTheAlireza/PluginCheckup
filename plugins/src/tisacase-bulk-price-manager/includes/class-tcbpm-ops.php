...
if ( ! class_exists( 'TCBPM_OPS' ) ) {

	final class TCBPM_OPS {
		...
		private static function percent_cap_for( $op ) {
			...
			return in_array( $op, $cap100, true ) ? 100.0 : TCBPM_Core::PERCENT_CEIL;
		}
		...
		public static function args_from_post( $post ) {
			...
			if ( empty( $post['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $post['nonce'] ) ), TCBPM_Core::NONCE ) ) {
			...
			if ( self::KIND_NONE === $kind ) {
				...
				if ( self::KIND_PERCENT === $kind ) {
					...
					if ( $value > TCBPM_Core::max_amount() ) {
						return new WP_Error( 'value', 'مبلغ واردشده از سقف مجاز (' . number_format( TCBPM_Core::max_amount() ) . ') بیشتر است.' );
					}
				}
				$args['value'] = $value;
			}
		...
		public static function wholesale_price_raw( $product_id ) {
			$raw = get_post_meta( absint( $product_id ), TCBPM_WHOLESALE_META, true );
		...
		public static function update_wholesale_meta( $product_id, $value ) {
			...
			if ( '' === $value ) {
				if ( metadata_exists( 'post', $product_id, TCBPM_WHOLESALE_META ) ) {
					delete_post_meta( $product_id, TCBPM_WHOLESALE_META );
			...
			$current = (string) get_post_meta( $product_id, TCBPM_WHOLESALE_META, true );
			if ( (string) $current === (string) $value ) {
				return false;
			}
			update_post_meta( $product_id, TCBPM_WHOLESALE_META, $value );
			clean_post_cache( $product_id );
			return true;
		}
		...
		public static function run_next_page( $run ) {
			...
			TCBPM_DB::insert_log( (int) $run['id'], $entries );
			...
			TCBPM_DB::update_run( (int) $run['id'], $counts );
		...
		public static function rollback_next_page( $run ) {
			...
			$batch_size = max( 1, min( 100, $batch_size ) );
			$total_rows = TCBPM_DB::count_log( $source_id );
			...
			$rows   = TCBPM_DB::get_log_page( $source_id, $offset, $batch_size );
			$updated = $skipped = 0;
			$errors  = array();
			$entries = array();
		...
		public static function finalize_run( $run_id, $status ) {
			...
			if ( $run && in_array( $run['status'], array( 'running', 'interrupted' ), true ) ) {
				TCBPM_DB::update_run( $run_id, array(
					'status'      => $status,
					'finished_at' => TCBPM_DB::now(),
					'updated_at'  => TCBPM_DB::now(),
				) );
			}
			// قیمت‌ها تغییر کرده‌اند؛ جدول lookup را برای نمایش صحیح فیلترها/بلوک‌های قیمتی به‌روز کن.
			if ( TCBPM_Core::wc_active() && function_exists( 'wc_update_product_lookup_tables' ) ) {
