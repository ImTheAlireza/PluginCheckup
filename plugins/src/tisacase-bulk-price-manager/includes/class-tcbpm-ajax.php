...
if ( ! class_exists( 'TCBPM_Ajax' ) ) {

	final class TCBPM_Ajax {
		...
		private static function guard() {
			...
			if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), TCBPM_Core::NONCE ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput -- درون شرط
		...
		public static function ajax_preview() {
			$args = TCBPM_OPS::args_from_post( wp_unslash( $_POST ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
			if ( is_wp_error( $args ) ) {
				self::send_wp_error( $args );
			}

			$parents = TCBPM_DB::selection_parent_ids( $args );
			if ( empty( $parents ) ) {
				wp_send_json_error( array( 'message' => 'هیچ محصولی در محدودهٔ انتخاب‌شده پیدا نشد (فیلترها را بررسی کن).' ), 400 );
			}

			$analysis = TCBPM_DB::analyze_targets( $parents, $args['operation'], TCBPM_Core::sample_size() );

			// ساخت نمونه‌ها با همان فرمول اجرا.
			$samples = array();
			foreach ( $analysis['samples'] as $oid => $info ) {
				$samples[] = TCBPM_OPS::sample_row( $oid, $info, $args['operation'], $args['value'] );
			}

			wp_send_json_success( array(
				'preview_token'      => TCBPM_OPS::make_token( $args ),
				...
				'operation_label'    => TCBPM_OPS::op_label( $args['operation'] ),
				'samples'            => $samples,
			) );
		}
		...
		public static function ajax_run() {
			...
			if ( $run_id ) {
				...
				if ( 'running' === $run['status'] && ! empty( $_POST['resume'] ) ) {
					...
					TCBPM_DB::update_run( $run_id, array( 'status' => 'running', 'updated_at' => TCBPM_DB::now() ) );
					$run['status'] = 'running';
				} elseif ( 'running' !== $run['status'] ) {
					wp_send_json_error( array( 'message' => 'اجرا در وضعیت قابل‌ادامه نیست (' . TCBPM_Core::translation( $run['status'] ) . ').' ), 409 );
				}

				$busy = TCBPM_DB::busy_slot( $run_id );
				if ( ! empty( $busy['active'] ) ) {
					wp_send_json_error( array( 'message' => 'اجرای دیگری در حال اجراست (#' . (int) $busy['active']['id'] . ').' ), 409 );
				}
				TCBPM_DB::update_run( $run_id, array( 'updated_at' => TCBPM_DB::now() ) );
			...
			if ( ! TCBPM_OPS::verify_token( $args, $preview_token ) ) {
			...
			$batch = max( 1, absint( TCBPM_Core::batch_size() ) );
			$pages = max( 1, (int) ceil( count( $parents ) / $batch ) );
			$args_store = $args;
			$args_store['batch'] = $batch; // ثابت‌ماندن مرز صفحه‌ها در طول اجرا.
			$schedule = ! empty( $_POST['schedule'] ) && ! empty( TCBPM_Core::setting( 'scheduled' ) );
			...
			if ( $schedule ) {
				...
				$run_id = TCBPM_DB::create_run( array(
					'type' => 'scheduled',
					'status' => 'queued',
					'user_id' => get_current_user_id(),
					'operation' => $args['operation'],
					'value' => $args['value'],
					'args_hash' => TCBPM_OPS::args_hash( $args ),
			...
			$busy2 = TCBPM_DB::busy_slot( $run_id );
			if ( ! empty( $busy2['active'] ) ) {
				TCBPM_DB::delete_run( $run_id );
		...
		private static function process_and_respond( $run_id ) {
			...
			$res = TCBPM_OPS::run_next_page( $run );
			if ( ! $res['ok'] ) {
				TCBPM_DB::update_run( $run_id, array( 'status' => 'failed', 'last_error' => $res['msg'], 'updated_at' => TCBPM_DB::now() ) );
			...
			if ( $data['done'] ) {
				TCBPM_OPS::finalize_run( $run_id, 'done' );
				$run = TCBPM_DB::get_run( $run_id );
		...
		public static function ajax_finish() {
			self::guard();
			$run_id  = isset( $_POST['run_id'] ) ? absint( $_POST['run_id'] ) : 0;
			$run = $run_id ? TCBPM_DB::get_run( $run_id ) : null;
			if ( ! $run ) {
				wp_send_json_error( array( 'message' => 'اجرا پیدا نشد.' ), 404 );
			}
			if ( (int) $run['user_id'] !== get_current_user_id() && ! current_user_can( 'manage_options' ) ) {
			...
			TCBPM_OPS::finalize_run( $run_id, 'stopped' );
		...
		public static function ajax_rollback_start() {
			...
			$total_log = TCBPM_DB::count_log( $source_id );
			if ( $total_log < 1 ) {
			...
			$new_id = TCBPM_DB::create_run( array(
				'type' => 'rollback',
				'status' => 'running',
				'user_id' => get_current_user_id(),
			...
			$busy2 = TCBPM_DB::busy_slot( $new_id );
			if ( ! empty( $busy2['active'] ) ) {
				TCBPM_DB::delete_run( $new_id );
		...
		public static function ajax_rollback_page() {
			...
			$res = TCBPM_OPS::rollback_next_page( $run );
			if ( ! $res['ok'] ) {
				TCBPM_DB::update_run( $run_id, array( 'status' => 'failed', 'last_error' => $res['msg'], 'updated_at' => TCBPM_DB::now() ) );
				wp_send_json_error( array( 'message' => $res['msg'] ), 500 );
			}
			...
			if ( $data['done'] ) {
				...
				$src = TCBPM_DB::get_run( (int) $run['parent_run_id'] );
				if ( $src && in_array( $src['status'], array( 'done', 'stopped' ), true ) ) {
					TCBPM_DB::update_run( (int) $src['id'], array( 'status' => 'rolled_back', 'updated_at' => TCBPM_DB::now() ) );
				}
				TCBPM_OPS::finalize_run( $run_id, 'done' );
			}
			wp_send_json_success( array(
		...
		public static function ajax_export_csv() {
			...
			if ( ! $run_id || ! TCBPM_Core::can() ) {
				wp_die( 'دسترسی غیرمجاز است.' );
			}
			if ( empty( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'tcbpm_export_' . $run_id ) ) {
			...
			$log_count = TCBPM_DB::count_log( $run_id );
			nocache_headers();
			header( 'Content-Type: text/csv; charset=utf-8' );
			header( 'Content-Disposition: attachment; filename="tcbpm-run-' . (int) $run_id . '-' . gmdate( 'Ymd-His' ) . '.csv"' );
			echo "\xEF\xBB\xBF"; // BOM برای اکسل فارسی.

			$out = fopen( 'php://output', 'w' );
			$op_label = TCBPM_OPS::op_label( $run['operation'] );

			fputcsv( $out, array( 'شماره اجرا', (int) $run_id ) );
			fputcsv( $out, array( 'نوع', TCBPM_Core::translation( $run['type'] ) ) );
			fputcsv( $out, array( 'عملیات', $op_label ) );
			fputcsv( $out, array( 'وضعیت', TCBPM_Core::translation( $run['status'] ) ) );
			...
			$row_no = 0;
			while ( true ) {
				$rows = TCBPM_DB::get_log_page( $run_id, $offset, $step );
				if ( empty( $rows ) ) {
					break;
				}
				$obj_ids = array();
				...
				$titles = TCBPM_DB::title_map( array_unique( $obj_ids ) );
				$skus   = TCBPM_DB::sku_map( array_unique( $obj_ids ) );
		...
		public static function ajax_search_wholesale_products() {
			...
			$ids = $wpdb->get_col(
				$wpdb->prepare( $sql, TCBPM_WHOLESALE_META, TCBPM_WHOLESALE_META, $like, $numeric_id, $like, $like, $limit )
			);
			$products = array();
			foreach ( $ids as $id ) {
