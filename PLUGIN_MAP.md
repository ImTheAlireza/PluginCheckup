...
- **tisacase-bulk-variation-manager 1.1.0** — قواعد واقعی فهرست مدل‌ها + بازسازی کامل: (۱) `sanitize_model_list()` دیگر کاما را جداکننده نمی‌گیرد؛ جداکننده‌ها «|» و خط جدید هستند و کاما داخل نام مدل می‌ماند (`iPhone 7,8,SE`, `A30s,A50,A50s`, `Redmi Note 9s,9 Pro`) — علت گزارش «۱۵۴ مدل» به‌جای ۱۱۲ مدل همین شکستن مدل‌ها بود؛ فهرست‌های کاماجدای قدیمی فقط با الگوی «کاما + فاصله» پشتیبانی می‌شوند تا تک‌مدل کامادار نشکند؛ `unique_models()` تکراری‌ها را با نام نرمال‌شده حذف می‌کند. (۲) `do_sync_preset()` در حالت `sync_rebuild` (پیش‌فرض روشن) بازسازی کامل می‌کند: همهٔ متغیرهای آن ویژگی حذف، ویژگی از والد برداشته، سپس با همان نام از روی فهرست جدید ساخته می‌شود؛ ترکیب سایر ویژگی‌ها از متغیرهای قبلی حفظ می‌شود (سقف ایمنی ۵۰۰۰ متغیر). (۳) `resolve_model_taxonomy()` ویژگی «مدل» را هوشمند پیدا می‌کند (نام دقیق → برچسب ویژگی‌های ووکامرس → هم‌پوشانی با مدل‌های خود محصول → تنها ویژگی متغیر محصول → ساخت با اجازهٔ صریح). (۴) `recreate_variation()` در Rollback، متغیرهای حذف‌شده را از اسنپ‌شات بازمی‌سازد (SKU اصلی فقط اگر آزاد باشد). (۵) شمارندهٔ زندهٔ «N مدل شناسایی شد» در فرم با همان منطق سرور؛ همچنان بدون ساخت/تغییر SKU و بدون محصول جدید.
- **tisacase-bulk-variation-manager 1.1.1** — رفع انفجار ترکیبی و محافظ زودهنگام: (۱) `do_sync_preset()` در حالت پیش‌فرض دقیقاً **یک متغیر به‌ازای هر مدل** می‌سازد (۱۱۲ مدل = ۱۱۲ متغیر) و دیگر در ترکیب سایر ویژگی‌ها ضرب نمی‌شود (علت اعداد ۱۹۵۵۸/۳۴۴۹۶)؛ (۲) **محافظ ایمنی پیش از هر تغییری** بررسی می‌شود (سقف ۳۰۰۰ ترکیب و ۲۰۰۰ مدل) — پیش‌تر بعد از حذف متغیرها فعال می‌شد و محصول را بدون متغیر رها می‌کرد؛ (۳) سایر ویژگی‌های متغیر در این حالت از والد برداشته می‌شوند تا ووکامرس منتظر مقدارشان نماند (ترم و داده حذف نمی‌شود؛ `recreate_variation` در Rollback همه را برمی‌گرداند)؛ (۴) گزینهٔ جدید `sync_keep_other_attrs` (پیش‌فرض خاموش) برای محصول‌های چندویژگی؛ (۵) `sync_product_attribute(...,$prev_attr)` نمایان‌بودن/ترتیب ویژگی را حفظ می‌کند؛ (۶) تأییدیهٔ آگاهانه در فرم: «N مدل شناسایی شد؛ همهٔ متغیرهای فعلی حذف و N متغیر ساخته می‌شود».
- **tisacase-bulk-variation-manager 1.2.0** — «همگام‌سازی کامل» حالا **تفاوتی** است (پیش‌فرض): `do_sync_preset()` بین `sync_diff()` و `sync_rebuild_full()` تصمیم می‌گیرد (`sync_rebuild` پیش‌فرض ۰). در `sync_diff()` سه دسته ساخته می‌شود — **keep** (مدل موجود در فهرست: دست‌نخورده، قیمت/موجودی حفظ)، **add** (ساخته می‌شود)، **remove** (خارج از فهرست: حذف می‌شود)؛ قیمت/موجودی فقط اگر کاربر مقدار داده باشد روی keepها اعمال می‌شود؛ `existing_other_attr_maps()` ترکیب سایر ویژگی‌ها را از متغیرهای موجود کپی می‌کند (بدون ضرب، حداکثر ۸ الگو)؛ ویژگی والد با ترتیب فهرست و حفظ visible/position بازنویسی می‌شود؛ محافظ ایمنی پیش از تغییر؛ `preview()` هم تفاوتی شد (فقط اختلاف‌ها + پیام «N مدل دست‌نخورده می‌ماند»)؛ `model_key()` کش شد. پاسخ مستقیم به «مدل‌های یکسان را چرا دوباره می‌سازد؟».
- **tisacase-brand-variations 1.0.0** (افزونهٔ دوازدهم) — پاسخ به «لیست ۱۵۴ مدلی TS184 خیلی شلوغ است»: دسته‌بندی گزینه‌های ویژگی متغیر بر اساس **برند** در صفحهٔ محصول (آیفون → خط جداکننده → سامسونگ → خط جداکننده → شیائومی) با پنل جستجوپذیر، رنگ برند و **سواچ ویژگی «رنگ»**. معماری: `includes/class-tcbv-settings.php` (یک آپشن `tcbv_settings`، پاک‌سازی کامل، کتابخانهٔ ۱۶ برند آماده)، `includes/class-tcbv-rules.php` (موتور: نرمال‌سازی نیم‌فاصله/`ي ى ی`/`ك ک`/ارقام فارسی + کلیدواژه‌ای‌کلمه‌ای + regex + فهرست دستی + گروه‌بندی `unknown`)، `includes/class-tcbv-frontend.php` (فیلتر `woocommerce_dropdown_variation_attribute_options_html`: مرتب‌سازی و `<optgroup>` سرور + enqueue)، `includes/class-tcbv-admin.php` (پنل ۴تبی + AJAX `tcbv_products`/`tcbv_product_attrs`/`tcbv_classify` + پیش‌نمایش زنده با همان CSS/JS فرانت‌اند)، `assets/frontend.js` (پنل بدون jQuery: جستجو، شمارش، چیپ فیلتر برند، جداکننده، سواچ رنگ، ناوبری کلیدی؛ select اصلی ووکامرس فقط با `.tcbv-native` پنهان می‌شود و پنل مقدار/رویداد `change` را همان‌طور شلیک می‌کند). **خط قرمز: هیچ محصول/متغیر/ویژگی/متایی نوشته نمی‌شود** (فقط نمایش؛ `uninstall.php` هم صرفاً آپشن را با اجازهٔ کاربر پاک می‌کند). قواعد پیش‌فرض روی لیست واقعی TS184: **۳۸ آیفون · ۶۴ سامسونگ · ۵۲ شیائومی · ۰ ناشناس · ۰ تضاد** (۱۵۴=۱۵۴) و روی لیست توسعه‌یافته (با Galaxy/Note/M31/Huawei/Nokia) هم صفر تضاد. آزمون‌ها (چون روی این سرور PHP نیست، با ابزارهای همین مخزن + jsdom): `tools/php-check.py` پاس، `node --check` روی هر دو JS پاس، و دو مجموعهٔ jsdom: ۲۲ سنجهٔ فرانت‌اند (ساخت سه گروه ۳۸/۶۴/۵۲، دو جداکننده، جستجوی `a55` → تک‌نتیجه و پنهان‌شدن جداکننده‌های خالی، انتخاب مدل → `select.value` + رویداد `change`، سواچ `#FF7EB6`/`#2F80ED`، چیپ شیائومی → ۵۲) و ۱۵ سنجهٔ پنل (ردیف‌های برند/کتابخانهٔ آماده/پیش‌نمایش زنده/رنگ زنده/نقشهٔ رنگ). پیش‌نمایش بدون وردپرس: `design/preview/brand-variations.html` (همان CSS/JS افزونه + ۱۵۴ مدل واقعی، با کنترل زندهٔ جداکننده/چیدمان/رنگ). زیپ: `plugins/dist/tisacase-brand-variations.zip` (۵۶KB · ۱۲ فایل).
- **tisacase-brand-variations 1.1.0** — **حالت تست** (درخواست کاربر: «اول روی دوتا محصولی که خودم انتخاب میکنم اعمال شه برا تست»): بخش تازهٔ `test` در `tcbv_settings` با `enabled` (پیش‌فرض **۱** — بعد از نصب/بازنشانی افزونه روی هیچ محصولی اجرا نمی‌شود تا محصول آزمایشی انتخاب شود)، `products` (فهرست شناسه، نرمال‌شده با `preg_match_all('/\d+/')`، سقف ۲۰) و `badge`. `TCBV_Settings::test_mode_on()`/`test_products()` و در `TCBV_Frontend::product_in_scope()` این حالت بر همهٔ دامنه‌ها اولویت دارد (نه پنل، نه enqueue، نه optgroup). **رفع باگ قدیمی**: `dropdown_html()` تا قبل از این نسخه دامنهٔ محصولات را نادیده می‌گرفت و `<optgroup>` را روی همهٔ محصولات می‌ساخت؛ حالا با `args_product_id( $args )` (product object → product_id → queried object) همان `product_in_scope()` را چک می‌کند. پنل: کارت «حالت تست» **بالای همهٔ تب‌ها و داخل همان فرم** (چیپ محصول + «مشاهده» + «×»، جستجو با AJAX `tcbv_products` که حالا `link` هم برمی‌گرداند، سقف ۲۰، افزودن تکراری = فلش)، پیل وضعیت در هیرو، هشدار «حالت تست روشن است ولی محصولی انتخاب نشده» و هشدار اطلاعاتی تعداد محصول، به‌همراه دکمهٔ «روشن‌کردن برای کل فروشگاه» (خاموش‌کردن تیک + ارسال فرم). نشان `tcbv-test-badge` روی صفحهٔ محصول با هوک `woocommerce_before_variations_form` و **فقط** برای `current_user_can('manage_woocommerce')`. دنبال‌کردن `test` در `handle_save` هم اضافه شد. پیش‌نمایش‌های بدون وردپرس: `design/preview/brand-variations.html` (کاربر) و `design/preview/brand-variations-admin.html` (ماکت پنل/کارت تست با CSS/JS واقعی). آزمون: `test-admin.js` ۲۷ سنجه (۱۲ سنجهٔ تازه برای حالت تست: وضعیت، پیل، جستجو با fetch ماک، افزودن/تکرار/حذف چیپ، خاموش‌کردن) و `test-admin-mock.js` ۱۲ سنجه روی خود ماکت؛ همه سبز. زیپ: ۶۴KB · ۱۲ فایل.
  - **لینک مستقیم زیپ (همیشه آخرین نسخهٔ این افزونه):** `https://raw.githubusercontent.com/ImTheAlireza/TisaCaseHub/main/plugins/dist/tisacase-brand-variations.zip` — تا قبل از مرج PR، شاخهٔ کاری: `…/arena/01a0cfce-tisacasehub/plugins/dist/tisacase-brand-variations.zip`
  - **صفحهٔ ریلیز:** `https://github.com/ImTheAlireza/TisaCaseHub/releases/tag/tisacase-brand-variations-1.1.0`
  - **شناخت هاب:** نیازی به به‌روزرسانی هاب نیست؛ هاب افزونه را از هدر `TisaCase Hub:` خودش می‌شناسد (کلید `tisacase-brand-variations`، گروه «محصول و محتوا»، آیکون `grid`). آپدیت از کارت هاب: زیپ همین لینک را بارگذاری کنید؛ پوشهٔ داخل زیپ `tisacase-brand-variations` است و با پوشهٔ نصب‌شده یکی می‌شود.
- **tisacase-hub 1.7.1** — «نصب از مخزن»: هر کارتی که افزونه‌اش روی سرور نیست، دکمهٔ **«نصب از مخزن»** می‌گیرد — `TSH_Registry::zip_url()` آدرس را از پایهٔ `ZIP_BASE` (یا `zip_base` تنظیمات) می‌سازد، `handle_install()` زیپ را با `download_url()` می‌گیرد، پوشهٔ داخل زیپ را با پوشهٔ مورد انتظار چک می‌کند (`ZipArchive`)، با `Plugin_Upgrader::install( overwrite_package=false )` نصب و در صورت داشتن دسترسی همان‌جا فعال می‌کند (`installed` / `installed_off` / `inst_failed`). کارت **«گروه‌بندی متغیرها بر اساس برند»** (کلید `brandvars`, dir `tisacase-brand-variations`) به `defaults()` اضافه شد تا پیش از نصب هم کارت داشته باشد (پیش‌تر فقط پس از نصب از هدر خودش کشف می‌شد). دو افزونهٔ بازنشسته (`tcbpm`/`pm`) با `'zip' => false` دکمهٔ نصب نمی‌گیرند. فیلترهای `tisacase_hub_zip_base`/`tisacase_hub_zip_url` + کادر تنظیمات `zip_base`. زیپ `plugins/dist/wc-telegram-orders.zip` با نام استاندارد ساخته شد (پروندهٔ قدیمی «(11)» فقط آرشیو شد). ابزار تازه: `tools/hub-check.py` (چک ایستا: اکشن فرم‌ها ↔ `admin_post_*`، جفت‌شدن nonce فرم/`check_admin_referer`، وجود زیپ برای هر آیتم، یکسان‌بودن نسخه در هدر/`TSH_VERSION`/`Stable tag`/زیپ) — همه سبز. نسخه ۱٫۷٫۱ (بالاتر از ۱٫۷٫۰ نصب‌شده روی سایت، تا نصب آن بازگشت به عقب نباشد).
